...
import static tomasulo.Tomasulo.mulModel;
...
public class mulUnit {
    ...
    public static void addWork(String s) {
        ...
        mulModel.addElement(work[busy].toString());
        busy++;
    }
    ...
    static void execute() {
        ...
        if (time >= rule) {
            time = 0;
            String temp = mulModel.getElementAt(0).toString().split(" ")[0];
            mulModel.removeElementAt(0);
            int t = operation.removeRedundancy(calculate(temp, work[0].op1, work[0].op2), temp, "MUL", "DIV");
